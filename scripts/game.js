function resetGameStatus() {

    activePlayer = 0;
    currentRound = 1;
    isGameOver = false;

    activePlayerNameElement.parentElement.style.display = 'block';
    gameOverElement.firstElementChild.innerHTML = 'You Won, <span id="winner-name">PLAYER NAME</span>!';
    gameOverElement.style.display = 'none';
    let gameBoardIndex = 0;
    for(let i=0; i<3; i++) {
        for(let j=0; j<3; j++) {
            gameData[i][j] = 0;
            gameFeildElements[gameBoardIndex].innerText = '';
            gameFeildElements[gameBoardIndex].classList.remove('disabled');
            gameBoardIndex++;
        }
    }
}

function startNewGame() {

    if (players[0].name === '' || players[1].name === '') {
        alert('Please set a custom player name for both Players!');
        return;
    }

    resetGameStatus();

    activePlayerNameElement.innerText = players[activePlayer].name;

    gameAreaElement.style.display = 'block';
}


function switchPlayers() {
    if(activePlayer === 0) {
        activePlayer = 1;
    } else {
        activePlayer = 0;
    }
    activePlayerNameElement.innerText = players[activePlayer].name;
}

function selectGameFeild(event) {
    // if(event.target.tagName !== 'LI') {
    //     return;
    // }

    if(isGameOver) {
        return;
    }
    
    const selectedColumn = +event.target.dataset.col - 1;
    const selectedRow = +event.target.dataset.row - 1;
    
    if (gameData[selectedRow][selectedColumn] > 0) {        //to ensure an empty feild is selected
        alert('Please select an empty feild');
        return ;
    }
    
    event.target.innerText = players[activePlayer].symbol;
    event.target.classList.add('disabled');

    
    gameData[selectedRow][selectedColumn] = activePlayer + 1;

    const winnerId = checkForGameOver();

    if(winnerId !== 0) {
        endGame(winnerId);
    }
    
    currentRound++;
    switchPlayers();
}

function checkForGameOver() {
    //Row check for equality
    for (let i=0; i<3; i++) {
        if(gameData[i][0] > 0 &&
           gameData[i][0]===gameData[i][1] &&
           gameData[i][1]===gameData[i][2]) {
            return gameData[i][0];
        }
    }
    //column check for equality
    for (let i=0; i<3; i++) {
        if(gameData[0][i] > 0 &&
           gameData[0][i]===gameData[1][i] &&
           gameData[0][i]===gameData[2][i]) {
            return gameData[0][i];
        }
    }
    //diagonal check top topLeft-bottomRight
    if(gameData[0][0] > 0 &&
       gameData[0][0] === gameData[1][1]&&
       gameData[1][1] === gameData[2][2]) {
        return gameData[0][0];
    }
    //diagonaal check bottomLeft-topRight
    if(gameData[2][0] > 0 &&
       gameData[2][0] === gameData[1][1]&&
       gameData[1][1] === gameData[0][2]) {
        return gameData[2][0];
    }

    if(currentRound === 9) {
        return -1;
    }

    return 0;
}

function endGame(winnerId) {
    isGameOver= true;

    gameOverElement.style.display = 'block';

    if(winnerId > 0) {
        const winnerName = players[winnerId-1].name;
        gameOverElement.firstElementChild.firstElementChild.innerText = winnerName;
    } else {
        gameOverElement.firstElementChild.innerText = 'It\'s a draw!';
    }
    activePlayerNameElement.parentElement.style.display = 'none';
}